...
type Props = VideoDetailPageParams["params"] & React.HTMLAttributes<HTMLDivElement>;

export const VideoDetailDescription = ({ videoId, ...props }: Props) => {
  const {
    data: { detail: videoDetail },
  } = useGetVideosDetail({ videoId });
  const { viewCountDisplayText, viewCount, publishedAtDisplayText, description, publishedAt } = videoDetail;

  const [showMore, setShoMore] = useState<boolean>(false);
