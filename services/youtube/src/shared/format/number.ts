...
export const formatNumberToKoreanText = (value: number, onlyHighestUnit?: boolean): string => {
  if (value === 0) {
    return "0";
  }
  const parts = [];
  ...
  parts.forEach((part, index) => {
    if (part > 0) {
      const partString = index === 0 ? formatNumberToKoreanWithCommas(part) : part.toString();
