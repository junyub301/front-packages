...
const parseQueryParams = (params: URLSearchParams): GetShortsVideosListRequestParams => {
  return {
    pageToken: params.get("pageToken") ?? undefined,
  };
};

const mappingResponse = (data: youtube_v3.Schema$SearchListResponse): GetShortsVideosListResponse => {
  const lists =
    data?.items?.map(({ id }) => {
      return {
        videoId: id?.videoId ?? "",
